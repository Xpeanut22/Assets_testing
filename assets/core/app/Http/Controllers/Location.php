...
class Location extends Controller
{
    ...
    public function byid( Request $request ) {
        ...
        $data = DB::table('location')->where('id', $id)->where('is_delete', 0)->first();
    ...
    public function update(Request $request){
		...
		$update = DB::table( 'location' )->where( 'id', $id )->where('is_delete', 0)
		->update(
			[
			'name'          => $name,
	...
	public function delete( Request $request ) {
		$id = $request->input( 'id' );
		$delete = DB::table( 'location' )->where( 'id', $id )->where('is_delete', 0)->update(['is_delete' => 1, 'updated_at' => date("Y-m-d H:i:s")]);
            if ( $delete ) {
                $res['success'] = 'success';
            } else{
                $res['success'] = 'failed';
